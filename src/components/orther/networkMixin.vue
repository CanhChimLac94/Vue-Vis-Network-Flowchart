{addEdge: (edge, callback) => {
            ...
            const newLink = new LinkModel({
              type: this.$lego.CONST.DESIGN.TYPE_LINK,
              triggerType: headNode?.type !== this.$lego.CONST.DESIGN.TYPES_NODE.IF ? TRIGGER_TYPE.UNSELECTED : null,
            })
            ...
            try {
              callback({ ...edge, id: `${edge.from}:${edge.to}`, ...new EdgeConfigVisModel({}) })
      ...
      this.network.on('afterDrawing', ({ canvas }) => {
        ...
        if (this.detailsOn) {
          ...
          function findDescent(fromNode, toNode) {
            const diagonalAngle = abs(atan(toNode.shape.height / toNode.shape.width))
            const angle = abs(atan((toNode.y - fromNode.y) / (fromNode.x - toNode.x)))
          ...
          for (const { from, to, triggerType } of this.localLinks) {
            ...
            if (this.nodeSet[from]?.type !== this.$lego.CONST.DESIGN.TYPES_NODE.IF || !triggerTypeName) continue
            const textMeasurement = ctx.measureText(triggerTypeName)
            const lineHeight = textMeasurement.actualBoundingBoxAscent + textMeasurement.actualBoundingBoxDescent

            const descentFrom = findDescent(nodes[to], nodes[from])
            const descentTo = findDescent(nodes[from], nodes[to])
            const x = r((nodes[from].x + nodes[to].x - textMeasurement.width + descentFrom.dx + descentTo.dx) / 2)
            const y = r((nodes[from].y + nodes[to].y + lineHeight + descentFrom.dy + descentTo.dy) / 2)
        ...
        const drawCustomLinkPoint = (link, { from, to }) => {
          ...
          ctx.fillStyle =
            edges[link.id].selected /*nodes[start.id].selected || nodes[diamond.id].selected ||*/ ||
          ...
          if (to) {
            ...
            ctx.lineTo(diamond.x + x + translateXY, diamond.y + y - translateXY + translateXY / 2)
            ctx.lineTo(diamond.x + x + translateXY, diamond.y + y + translateXY - translateXY / 2)
            ctx.closePath()
            ctx.fill()
            ctx.restore()
        ...
        for (const link of Object.values(edges)) {
          if (this.nodeSet[link.fromId]?.type === this.$lego.CONST.DESIGN.TYPES_NODE.IF)
            drawCustomLinkPoint(link, { from: true })
          if (this.nodeSet[link.toId]?.type === this.$lego.CONST.DESIGN.TYPES_NODE.IF)
