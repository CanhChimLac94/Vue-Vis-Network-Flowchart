<template>
  <div ref="visualization" class="network"></div>
</template>
...
export default {
  ...
  computed: {
    mapSVG: () => {
      return document.querySelector(".network canvas");
    },
  },
  props: {
    edges: {
  ...
  methods: {
    init() {
      // allow drop
      this.mapSVG.addEventListener("dragover", (evt) => {
        evt.preventDefault();
      });

      this.mapSVG.addEventListener("drop", (evt) => {
        evt.preventDefault();
        this.$emit(translateEvent("onDrop"), evt);
      });
    },
    setData(n, e) {
